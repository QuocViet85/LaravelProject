    ...
    <form action="" method="POST">
        <div class="row">
            <div class="col-6">
                <div class="mb3">
                    <label for="">Tên</label>
                    <input type="text" name="name" class="form-control title @error('name') is-invalid @enderror" placeholder="Tên..." value="{{ old('name') ?? $course->name}}">
            ...
            <div class="col-6">
                <div class="mb3">
                    ...
                    <input type="text" name="slug" class="form-control slug @error('slug') is-invalid @enderror" placeholder="slug..." value="{{ old('slug') ?? $course->slug}}">
                    <div class="invalid-feedback">
                        @error('slug')
                        {{ $message }}
                        @enderror
                    ...
                    <select name="teacher_id" id="" class="form-select @error('teacher_id') is-invalid @enderror">
                        ...
                        @if ($teachers)
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('teacher_id') == $teacher->id || $teacher->id == $course->teacher_id ? 'selected' : false }}>{{ $teacher->name }}</option>
                            @endforeach
                        @endif
                    </select>
                    <div class="invalid-feedback">
            ...
            <div class="col-12">
                <div class="mb3">
                    ...
                    <textarea name="supports" class="form-control @error('sale_price') is-invalid @enderror" placeholder="Hỗ trợ...">{{ old('supports') ?? $course->supports}}</textarea>
                    <div class="invalid-feedback">
                        @error('supports')
                        {{ $message }}
                        @enderror
                ...
                <div class="col-12">
                    <div class="mb3">
                        ...
                        <textarea name="detail" class="form-control ckeditor @error('detail') is-invalid @enderror" placeholder="Nội dung...">{{ old('detail') ?? $course->detail}}</textarea>
                        <div class="invalid-feedback">
                            @error('detail')
                            {{ $message }}
                            @enderror
                ...
                <div class="mb3">
                    <div class="row align-items-{{ $errors->has('thumbnail') ? 'center' : 'end' }}">
                        <div class="col-7">
                            ...
                            <input type="text" name="thumbnail" class="form-control @error('thumbnail') is-invalid @enderror" placeholder="Ảnh đại diện..." value="{{ old('thumbnail') ?? $course->thumbnail }}" id="thumbnail">
                            <div class="invalid-feedback">
                                @error('thumbnail')
                                {{ $message }}
                                @enderror
                        ...
                        <div class="col-3" >
                            <div id="holder" style="margin-top:15px;max-height:100px;">
                                @if(old('thumbnail') || $course->thumbnail)
                                    <img src={{ old('thumbnail') ?? $course->thumbnail }} style="height: 5rem;">
                                @endif
                            </div>
                        </div>
                    </div>
